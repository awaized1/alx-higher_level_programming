#!/usr/bin/python3
'''Code module for the Base unit tests.'''
import unittest
...
class TestBase(unittest.TestCase):
    '''Function tests for Base class.'''

    def setUp(self):
        '''Function imports module and instantiates class'''
        Base._Base__nb_objects = 0
        pass

    def tearDown(self):
        '''Function cleans up test_method.'''
        pass

    def test_A_nb_objects_private(self):
        '''It tests if nb_objects is a private class attribute.'''
        self.assertTrue(hasattr(Base, "_Base__nb_objects"))

    def test_B_nb_objects_initialized(self):
        '''Code tests if nb_objects initializes to zero.'''
        self.assertEqual(getattr(Base, "_Base__nb_objects"), 0)

    def test_C_instantiation(self):
        '''Code tests the Base instantiation.'''
        b = Base()
    ...
    def test_D_constructor_args_2(self):
        '''The tests constructor with 2 notself args.'''
        with self.assertRaises(TypeError) as e:
...
were given"
    ...
    def test_E_consecutive_ids(self):
        '''Function tests consecutive ids.'''
        b1 = Base()
        b2 = Base()
        self.assertEqual(b1.id + 1, b2.id)

    def test_F_id_synced(self):
        '''The tests sync btw class and instance id.'''
        b = Base()
    ...
    def test_G_custom_id_int(self):
        '''Function tests custom int id.'''
        a = 98
        b = Base(a)
        self.assertEqual(b.id, a)

    def test_G_custom_id_str(self):
        '''Function tests the custom int id.'''
        a = "FooBar"
        b = Base(a)
        self.assertEqual(b.id, a)

    def test_G_id_keyword(self):
        '''Function tests the id passed as keyword arg.'''
        a = 421
        b = Base(id=a)
        self.assertEqual(b.id, a)
    ...
    def test_H_to_json_string(self):
        '''Code tests to_json_string signature:'''
        with self.assertRaises(TypeError) as e:
            Base.to_json_string()
...
"height": 34340}]'
    ...
    def test_I_save_to_file(self):
        '''Code tests the save_to_file() method.'''
        import os
        r1 = Rectangle(10, 7, 2, 8)
        r2 = Rectangle(2, 4)
    ...
    def test_K_load_from_file(self):
        '''Code tests the load_from_file() method.'''
        r1 = Rectangle(10, 7, 2, 8)
        r2 = Rectangle(2, 4)
        list_in = [r1, r2]
